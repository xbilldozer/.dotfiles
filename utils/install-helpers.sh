...
stow_uninstall() {
  ...
  if [[ -z $package_dir ]]; then
    echo "Must set package_dir as the base directory for the packages you want to uninstall"  
    return 1
  fi
  if [[ -z $target_dir ]]; then
    echo "Must set target_dir as the directory into which you wish to uninstall your pacakges (usually your home directory)"  
    return 1
  fi
  if [[ -z $packages ]]; then
    echo "Must provide packages as comma-delimited list of packages to uninstall"  
    return 1
  fi
  ...
  for folder in $(echo $packages | sed "s/,/ /g")
  do
      if ! stow -t $target_dir -D $folder; then
        echo "FAILED: un-stowing package $package_dir/$folder" >&2
        all_succeeded=false
      fi
  done
  popd
